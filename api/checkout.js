...
async function insertOrder(row) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("Supabase insert erro:", await res.text());
    } else {
...
async function getToken() {
  ...
  if (!res.ok) {
    throw new Error("Token falhou: " + JSON.stringify(data));
  }
...
async function tokenizeCard(token, cardNumber, customerId) {
  ...
  if (!res.ok) {
    throw new Error("Tokenização falhou: " + JSON.stringify(data));
  }
...
function detectBrand(num) {
  ...
  if (/^(6362|438935|504175|451416|636297|5067|4576|4011)/.test(n)) {
    return "Elo";
  }
...
module.exports = async function handler(req, res) {
  try {
    ...
    /* ── Validações ─────────────────────────── */
    if (!["credit", "debit", "pix"].includes(kind)) {
    ...
    const customerId =
      "customer-" + String(customerCpf).replace(/\D/g, "");
